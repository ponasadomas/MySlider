...
export function BreatherLotusSlide({ slideContents }: SlideBreatherProps) {
  ...
  const animationElement_squaresRef = Array(20)
    .fill(0)
  ...
  useEffect(() => {
    ...
    animationElement_squaresRef.forEach((ref, i) => {
      if (ref.current) {
        // Calculate 50% of the element's width and height
        const halfWidth = ((i + 1) * 4) / 2;
        const halfHeight = ((i + 1) * 4) / 2;
        ...
        const squareTimeline = gsap
          ...
          .fromTo(
            ref.current,
            {
              transform: `translate(-${halfWidth}px, -${halfHeight}px)`,
              rotate: (i + 1) * 36,
              scale: 1
            },
            {
  ...
  return (
    <div className="slider__breatherLotusSlide">
      <div className="slider__breatherLotusSlide-content">
        <section className="slider__section">
          <div
            ref={animationElement_containerRef}
            className="slider__breatherLotusSlide-container">
            {animationElement_squaresRef.map((ref, i) => {
              return (
                <div
                  key={i}
                  ref={ref}
                  className="slider__breatherLotusSlide-square"></div>
              );
            })}
          </div>
          ...
          <h2
            ...
            className="slider__breatherLotusSlide-headline">
            {loadingMessages[0]}
          </h2>
          <p>
            Sit back, relax, and take a few deep breaths. We’ll have everything
            ready shortly.
          </p>
        </section>
        <svg
          ...
          focusable="false"
          role="img"
          className="slider__breatherLotusSlide-svg">
          <clipPath
            id="logo-clip-path"
          ...
          <rect
            id="loading-bar-rect"
            className="slider__breatherLotusSlide-svgRect"

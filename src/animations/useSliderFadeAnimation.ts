...
export type SliderFadeAnimationType = () => void;
...
export const useSliderFadeAnimation = (onAnimationComplete: () => void) => {
  const {
    sliderSlideRef,
    setDisableMouseEvents
  } = useSliderContext();

  const animation: SliderFadeAnimationType = () => {
    let startX = '0%';
    let endX = '0%';

    // Animation for the fadeOut slide
    gsap.fromTo(

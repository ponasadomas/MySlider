...
export type SliderHorizontalAnimationType = (
...
export const useSliderHorizontalAnimation = (onAnimationComplete: () => void) => {
  const {
    sliderSlideRef,
    setDisableMouseEvents
  } = useSliderContext();

  const animation: SliderHorizontalAnimationType = (animationDirection) => {
    let startX = '0%';
    let endX = '0%';
    ...
    // Animation for the horizontal slide
    gsap.fromTo(
